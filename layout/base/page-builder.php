...
$folder = !empty($args['folder']) ? $args['folder'] : "layout/page-builder/";
$container_class = !empty($args['container_class']) ? " {$args['container_class']}" : " ";

$default_space = !empty($args['space']) ? $args['space'] : "noemal";
...
<?php while (have_rows("page_builder")) : the_row();
  ...
  $class = "tpf-block tpf-$layout tpf-style-$pb_style tpf-space-$pb_space";

  // If there is layout specifi class add it
  ...
  if ($n == 1) $class .= " tpf-block-first";
  if ($n == count($page_builder)) $class .= " tpf-block-last";

  ?>

  <div class="<?= $class ?>">

    <?php if ($pb_space != "no-space") : ?>
      <div class="uk-container<?= $container_class ?>">
      <?php endif; ?>

      <?php
      tpf_render("{$folder}{$layout}");
      wp_reset_postdata();
      ?>
